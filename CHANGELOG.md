...
### Changed
- **ZAPX is now free software, licensed under the GNU GPL v3 or later.** The
  `LICENSE` file carries the full GPLv3 text and the project metadata declares
  `GPL-3.0-or-later`. Bundled third-party components keep their own permissive
  licenses (see `THIRD-PARTY-NOTICES.md`). The README was also reorganized so the
  feature set reads more intuitively.
