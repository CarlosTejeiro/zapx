...
//! of [`ActiveSession`] is the `Arc<SshHandle>` reused for `direct-tcpip`.

use std::sync::Arc;

use tauri::State;

use core_transport::ForwardInfo;

use crate::error::AppError;
use crate::state::AppState;

/// Look up the `Arc<SshHandle>` for a live SSH session, or return a clear error.
fn session_handle(
    state: &AppState,
    session_id: &str,
) -> Result<Arc<core_transport::SshHandle>, AppError> {
